...
public class ActionSkillUsing : MonoBehaviour
{
    ...
    public void SaveSkillbar()
    {
        ...
        PlayerPrefs.SetString("ActionSkillBar", skillSlots);
        PlayerPrefs.Save();
    }

    public void LoadSkillBar()
    {
        string skillSlots = PlayerPrefs.GetString("ActionSkillBar");
    ...
    public void AssignButtons()
    {
        ...
        for (int i = 0; i < abilities.Length; i++)
        {
            ...
            if (abilities[i].tag == "Skill" && abilities[i] != null)
                abilities[i].GetComponentInChildren<UseSkills>().button = "Button" + (i + 1);
            if (abilities[i].GetComponentInChildren<UseSkills>().button == null)
                abilities[i].GetComponentInChildren<UseSkills>().button = "InactiveButton";

            if (abilities[i].GetComponentInChildren<SwitchSkills>().spellButton == "")
                abilities[i].GetComponentInChildren<SwitchSkills>().spellButton = "InactiveButton";

            if (abilities[i].GetComponentInChildren<SwitchSkills>().spellButton == null)
                abilities[i].GetComponentInChildren<SwitchSkills>().spellButton = "InactiveButton";
        }
    }
    ...
    void Update()
    {
        ...
        if (isCasting == false)
        {
            if (Input.GetButtonDown(skills[0].skillIcon.GetComponentInChildren<UseSkills>().button) ||
    Input.GetButtonDown(skills[0].skillIcon.GetComponentInChildren<SwitchSkills>().spellButton))
            {
            ...
            else if (Input.GetButtonDown(skills[2].skillIcon.GetComponentInChildren<UseSkills>().button) ||
                     Input.GetButtonDown(skills[2].skillIcon.GetComponentInChildren<SwitchSkills>().spellButton))
            {
            ...
            else if (Input.GetButtonDown(skills[3].skillIcon.GetComponentInChildren<UseSkills>().button) ||
                     Input.GetButtonDown(skills[3].skillIcon.GetComponentInChildren<SwitchSkills>().spellButton))
            {
            ...
            else if (Input.GetButtonDown(skills[4].skillIcon.GetComponentInChildren<UseSkills>().button) ||
                     Input.GetButtonDown(skills[4].skillIcon.GetComponentInChildren<SwitchSkills>().spellButton))
            {
                if (skills[4].skillTimer >= skills[4].skillCD)
                {
                    if (skills[4].isCastable)
                    {
                        skills[4].castSlider.gameObject.SetActive(true);
                        skills[4].castSlider.value = 0;


                        skills[4].castSlider.maxValue = skills[4].castTime;
                    }
                    else
                    {
                        Player.GetComponent<Ability1>().arcaneBallAbilityUse();
        ...
        if (skills[4].castSlider.value == skills[4].castSlider.maxValue)
        {
            Player.GetComponent<Ability1>().arcaneBallAbilityUse();
            isCasting = false;
            skills[4].skillTimer = 0; //start skill CD
            skills[4].castSlider.value = 0;
            skills[4].castSlider.gameObject.SetActive(false);
        }
        ...
        foreach (Skill s in skills)
        {
            ...
            if (s.castSlider.gameObject.active == true)
            {
                ...
                s.castSlider.value +=
                    Time.deltaTime + (stats.castSpeed * 0.001f); //fill up castbar dependence on castSpeed stats
                isCasting = true;
            }
